...
- [`architecture.md`](./docs/architecture.md) — stack, diagrams, file structure
- [`database.md`](./docs/database.md) — DB schema (tables, indexes, encryption)
- [`instagram-api.md`](./docs/instagram-api.md) — how the Instagram API works (verified in practice)
- [`vk-api.md`](./docs/vk-api.md) — how the VK API works
- [`deployment.md`](./docs/deployment.md) — Docker, CI/CD, env vars, ngrok webhook testing
- [`plans.md`](./docs/plans.md) — subscription plans/limits + where users view logs
...
> **Phases 1–4 are implemented. Phase 5 (Polish) is in progress.**

The `backend/` and `frontend/` directories now exist on disk with working code:
backend builds/vets/tests clean, frontend builds clean. See
[`handoff.md`](./docs/handoff.md) for the authoritative, up-to-date status
(what shipped per phase, known gaps, seed data, and verification commands).

- **Foundation, Subscriptions/Admin, Instagram (+ engine core), VK (adapter)** — done.
- **Polish** — partially shipped (test-trigger endpoint, DnD priority, plan-limit UX,
  IG token-refresh + log-retention crons, skipped-log UI, golangci-lint + CI).
  Remaining/deferred: expanded VK unit tests, WebSocket realtime UI, production
  Docker/GHCR, frontend tests.

Follow the structure in [`architecture.md`](./docs/architecture.md). New work continues
in the phase order: **Foundation → Subscriptions → Instagram (+ engine core) → VK (adapter) → Polish**
(see [`plan.md`](./docs/plan.md)). Before extending a subsystem, read the relevant
"Critical files — extend, do not duplicate" entry in [`handoff.md`](./docs/handoff.md).

Note: `CLAUDE.md` is the Claude Code copy of `docs/agents.md` (doc paths adjusted).
The convention sections are kept in sync; `docs/agents.md` does not carry this
repo-state section. Keep the two in sync when editing conventions.

---

## Architecture at a Glance

Monorepo: **Go backend** (two binaries) + **React/Vite frontend**.

- `cmd/api` — Gin HTTP server: REST API + WebSocket + Meta webhook receiver.
- `cmd/worker` — Bot Engine: `WorkerManager` runs one `AccountWorker` goroutine per active account.
- **Instagram** is push-based (Meta webhook → Asynq task → reply). **VK** is pull-based (Bots Long Poll goroutine).
- **PostgreSQL** (all persistent data), **Redis** (sessions, rate limiters, trigger cache, Asynq queue, pub/sub).
...
| Layer | Choice |
|-------|--------|
| Backend | Go 1.22+, Gin, pgx/v5 + **sqlc**, goose migrations, Asynq, zap, viper, gorilla/websocket |
| VK | `github.com/SevereCloud/vksdk/v3` (Long Poll bot) |
| Frontend | React 18, Vite 5, TypeScript 5, shadcn/ui + Tailwind, TanStack Query (server state), Zustand (client state), React Router v6, axios, React Hook Form + zod |
| Infra | PostgreSQL 16, Redis 7, Docker Compose, GitHub Actions → GHCR |

---

## Development Commands

> These assume the structure from `architecture.md` once scaffolded. Run from repo root unless noted.

```bash
# Bring up infra
docker compose up -d postgres redis

# Run full dev stack (api + worker + frontend with hot reload)
docker compose up api worker frontend

# --- Backend (cd backend) ---
go run ./cmd/api                 # HTTP server (:8080)
go run ./cmd/worker              # Bot Engine
go test ./...                    # all tests (CI runs this)
go test ./internal/engine/...    # single package
go vet ./... && gofmt -l .       # lint / format check
sqlc generate                    # regenerate internal/db/generated from query/*.sql (do NOT hand-edit generated/)
goose -dir internal/db/migrations postgres "$DATABASE_URL" up    # apply migrations
...
# --- Webhook testing (Meta requires a public URL) ---
ngrok http 8080                  # then set META_CALLBACK_URL=https://<id>.ngrok.io/webhooks/instagram
```

Asynq queue monitor (dev): `http://localhost:8081` — standalone `asynqmon` container in docker-compose.
Required env vars are documented in [`deployment.md`](./docs/deployment.md); copy `.env.example` → `.env`.
...
Follow the structure from [`architecture.md`](./docs/architecture.md). Summary:
