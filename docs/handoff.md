...
10. **Production Docker + GHCR** — **partially shipped**. `.github/workflows/release.yml` builds + pushes `api`/`worker`/`frontend` images to GHCR on a `v*.*.*` tag (semver + `latest` tags via `docker/metadata-action`, `latest=auto` skips pre-releases) and cuts a GitHub Release with auto-generated notes via `softprops/action-gh-release`. The backend Dockerfile now has a `production` (api) + `production-worker` target so the two binaries ship as distinct runnable images. **Still deferred**: auto-deploy to a host (SSH / `workflow_dispatch`) and a real `docker-compose.prod.yml` (only documented in `deployment.md` so far) — both blocked on a deploy-target decision (Fly.io? Hetzner? Render?).
...
| #10 Production Docker images + GHCR | **partial** | Build+push to GHCR on `v*.*.*` tag + GitHub Release shipped (`release.yml`; backend gained a `production-worker` target). Remaining: auto-deploy + real `docker-compose.prod.yml`, blocked on deploy-target choice. |
