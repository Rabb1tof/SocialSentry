...
10. **Production Docker + GHCR** — **shipped**. `.github/workflows/release.yml` builds + pushes `api`/`worker`/`migrate`/`frontend` images to GHCR on a `v*.*.*` tag (semver + `latest` via `docker/metadata-action`, `latest=auto` skips pre-releases) and cuts a GitHub Release. Backend Dockerfile targets: `production` (api), `production-worker`, and `migrate` (goose + migration SQL). Real **`docker-compose.prod.yml`** (repo root) pulls these images — `postgres`, `redis`, one-shot `migrate` (goose up), `api`, `worker`, `frontend`; config via `${VAR}` interpolation + [`.env.prod.example`](../.env.prod.example). Frontend prod image serves the SPA and reverse-proxies `/api`/`/webhooks`/`/ws` to the internal api (single-origin; upstream via `API_UPSTREAM` → `frontend/nginx.conf.template`). EasyPanel deploy guide: [`easypanel.md`](./easypanel.md). **Still deferred**: auto-deploy on tag (SSH / `workflow_dispatch`) — left to the deploy target.

   > **CI fix (sqlc):** `internal/db/generated` is gitignored, so CI/Docker now regenerate it. `ci.yml` runs `sqlc generate@v1.31.1` before the Go steps; the backend Dockerfile `builder` runs the same before `go build`. Without this, `go build` / `docker build` from a clean checkout fail with "no required module provides package .../internal/db/generated".
...
| #10 Production Docker images + GHCR | **shipped** | release.yml pushes 4 images (api/worker/migrate/frontend) on `v*.*.*` + GitHub Release; `docker-compose.prod.yml` pulls them; EasyPanel guide in `docs/easypanel.md`. CI/Docker regenerate gitignored sqlc code. Remaining (optional): auto-deploy on tag. |
